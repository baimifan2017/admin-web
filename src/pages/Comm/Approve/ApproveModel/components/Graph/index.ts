...
export default class FlowGraph {
  ...
  public static init() {
    this.graph = new Graph({
      ...
      grid: {
        size: 10,
        visible: true,
        type: 'doubleMesh',  //网格样式 'dot' | 'fixedDot' | 'mesh'
        args: [
          {
            color: '#cccccc', // 主网格颜色
            thickness: 1, // 主网格宽度
      ...
      selecting: {
        ...
        movable: true,
        showNodeSelectionBox: true, // 是否显节点选择框
        filter: ['groupNode'], // 节点过滤器，被过滤的节点将不能被选中
      },
      connecting: {
        anchor: 'center',
        ...
        createEdge() {
          return new Shape.Edge({ // Rect-举行、Edge、Circle
            attrs: {
              line: {
                stroke: '#5F95FF',
                strokeWidth: 1,
  ...
  private static initEvent() {
    ...
    graph.on('node:collapse', ({ node, e }:any) => {
      ...
      cells.forEach((n: { hide: () => void; show: () => void }):any => {
        if (collapsed) {
          n.hide()
        } else {
          n.show()
